<?php
/*
 Name: Archia - Architecture & Interior HTML Templates
...
$dzEmailTo 		= "bazenisiriusplus@gmail.com";   /* Receiver Email Address */
$dzEmailFrom    = "Fermedik";
...
try {
    if (!empty($_POST)) {
        ...
        if (!isset($_POST['g-recaptcha-response'])) {
            $dzRes['status'] = 0;
			$dzRes['msg'] = 'ReCaptcha nije postavljeno.';
        ...
        if (!$response->isSuccess()) {
            $dzRes['status'] = 0;
			$dzRes['msg'] = 'ReCaptcha nije potvrđen.';
			echo json_encode($dzRes);
			exit;
        }
		...
		{
			$dzRes['status'] = 0;
			$dzRes['msg'] = 'Pogrešan format e-pošte.';
		}
		$dzMailSubject = 'Fermedik|Contact Form: Osoba koja želi kontaktirati';
		$dzMailMessage	= 	"
							Osoba želi da vas kontaktira: <br><br>
							Name: $dzName<br/>
							Email: $dzEmail<br/>
		...
		{
			$dzRes['status'] = 1;
			$dzRes['msg'] = 'Uspešno smo primili vašu poruku. Hvala na kontaktu.';
		}
		else
		{
			$dzRes['status'] = 0;
			$dzRes['msg'] = 'Problem u slanju pošte, pokušajte ponovo kasnije.';
		}
		echo json_encode($dzRes);
		exit;
    ...
    $dzRes['status'] = 0;
	$dzRes['msg'] = $e->getMessage().'Problem u slanju pošte, pokušajte ponovo kasnije.';
	echo json_encode($dzRes);
	exit;
}
